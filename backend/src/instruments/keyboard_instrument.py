        ...
        for i in range(len(position)-1):
            for j in range(i+1, len(position)):
                ...
                if distance > ok_distance:
                    ...
                    cost += add
                    if display:
                        print(f"Fingers {finger_i} and {finger_j} too far: \
        ...
        for i in range(len(pos_1_full)):
            if pos_1_full.placements[i] != -1 and pos_2_full.placements[i] != -1:
                ...
                if display and add > 0:
                    print(f"Finger {i}: {pos_1_full.placements[i]} \
-> {pos_2_full.placements[i]}, cost: {add}")
        ...
        if (max(position_1.fingers) < self.hands_separation \
            and min(position_2.fingers) >= self.hands_separation) \
            or (max(position_2.fingers) < self.hands_separation \
        ...
        for i, (placement1, finger1, placement2, finger2) \
            in enumerate(zip(
